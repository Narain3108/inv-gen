 ...
 * - Single Responsibility: Tab content separated into components
...
function GeneralTab({ form }: { form: UseFormReturn<CategoryFormData> }) {
...
function ProductsTab({
  ...
  fieldArray: UseFieldArrayReturn<CategoryFormData, 'products', 'id'>;
}) {
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button type="button" size="sm" onClick={handleAddProduct} className="gap-2">
...
export function CategoryForm({ category, onSubmit, onCancel }: CategoryFormProps) {
  ...
  // Tab navigation
  const tabsOrder: TabKey[] = ['general', 'products'];
  const fieldsPerTab: Record<TabKey, string[]> = {
    general: ['categoryName', 'defaultGstRate'],
    products: [],
  };
  ...
  const tabs: TabConfig[] = useMemo(() => [
    ...
    },
    {
      key: 'products',
      label: 'Products',
      content: <ProductsTab form={form} fieldArray={fieldArray} />,
    },
  ], [form, fieldArray]);
